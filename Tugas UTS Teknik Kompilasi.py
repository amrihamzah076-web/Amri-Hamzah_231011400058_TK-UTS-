...
class MiniCompiler:
    def __init__(self, source, env):
        # TUGAS 1: Perbarui regex agar mengenali simbol '^' dan operator dasar lainnya '*'
        self._tokens = iter(re.findall(r'[a-zA-Z]\w*|\d+(?:\.\d+)?|[+\-*/()^]', source) + [None])
        self._current = None
        self._env = env
        self._temp_count = 0
        self.advance()
    ...
    def term(self):
        ...
        node = self.power() # Berubah dari self.factor() menjadi self.power()
        while self._current in ('*', '/'):
            op = self._current
            self.advance()
            node = BinOp(left=node, op=op, right=self.power()) # Kanan juga disesuaikan
        return node
